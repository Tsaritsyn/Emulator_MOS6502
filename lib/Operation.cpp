...
std::expected<Operation, InvalidOperation> Emulator::decode(const ROM &memory, Word &PC, size_t &cycle) noexcept {
    Byte opCode = memory.fetch_byte_and_proceed(PC, cycle);

    switch (opCode) {
        default: return std::unexpected(InvalidOperation{.opCode = opCode});

        case ADC_IMMEDIATE:   return ADC_Immediate{.value = memory.fetch_byte_and_proceed(PC, cycle)};
        case ADC_ZERO_PAGE:   return ADC_ZeroPage{.address = memory.fetch_byte_and_proceed(PC, cycle)};
        case ADC_ZERO_PAGE_X: return ADC_ZeroPageX{.address = memory.fetch_byte_and_proceed(PC, cycle)};
        case ADC_ABSOLUTE:    return ADC_Absolute{.address = memory.fetch_word_and_proceed(PC, cycle)};
        case ADC_ABSOLUTE_X:  return ADC_AbsoluteX{.address = memory.fetch_word_and_proceed(PC, cycle)};
        case ADC_ABSOLUTE_Y:  return ADC_AbsoluteY{.address = memory.fetch_word_and_proceed(PC, cycle)};
        case ADC_INDIRECT_X:  return ADC_IndirectX{.address = memory.fetch_byte_and_proceed(PC, cycle)};
        case ADC_INDIRECT_Y:  return ADC_IndirectY{.address = memory.fetch_byte_and_proceed(PC, cycle)};

        case AND_IMMEDIATE:   return AND_Immediate{.value = memory.fetch_byte_and_proceed(PC, cycle)};
        case AND_ZERO_PAGE:   return AND_ZeroPage{.address = memory.fetch_byte_and_proceed(PC, cycle)};
        case AND_ZERO_PAGE_X: return AND_ZeroPageX{.address = memory.fetch_byte_and_proceed(PC, cycle)};
        case AND_ABSOLUTE:    return AND_Absolute{.address = memory.fetch_word_and_proceed(PC, cycle)};
        case AND_ABSOLUTE_X:  return AND_AbsoluteX{.address = memory.fetch_word_and_proceed(PC, cycle)};
        case AND_ABSOLUTE_Y:  return AND_AbsoluteY{.address = memory.fetch_word_and_proceed(PC, cycle)};
        case AND_INDIRECT_X:  return AND_IndirectX{.address = memory.fetch_byte_and_proceed(PC, cycle)};
        case AND_INDIRECT_Y:  return AND_IndirectY{.address = memory.fetch_byte_and_proceed(PC, cycle)};

        case ASL_ACCUMULATOR: return ASL_Accumulator{};
        case ASL_ZERO_PAGE:   return ASL_ZeroPage{.address = memory.fetch_byte_and_proceed(PC, cycle)};
        case ASL_ZERO_PAGE_X: return ASL_ZeroPageX{.address = memory.fetch_byte_and_proceed(PC, cycle)};
        case ASL_ABSOLUTE:    return ASL_Absolute{.address = memory.fetch_word_and_proceed(PC, cycle)};
        case ASL_ABSOLUTE_X:  return ASL_AbsoluteX{.address = memory.fetch_word_and_proceed(PC, cycle)};

        case BCC_RELATIVE:    return BCC{.offset = (char)memory.fetch_byte_and_proceed(PC, cycle)};
        case BCS_RELATIVE:    return BCS{.offset = (char)memory.fetch_byte_and_proceed(PC, cycle)};
        case BEQ_RELATIVE:    return BEQ{.offset = (char)memory.fetch_byte_and_proceed(PC, cycle)};
        case BNE_RELATIVE:    return BNE{.offset = (char)memory.fetch_byte_and_proceed(PC, cycle)};
        case BMI_RELATIVE:    return BMI{.offset = (char)memory.fetch_byte_and_proceed(PC, cycle)};
        case BPL_RELATIVE:    return BPL{.offset = (char)memory.fetch_byte_and_proceed(PC, cycle)};
        case BVC_RELATIVE:    return BVC{.offset = (char)memory.fetch_byte_and_proceed(PC, cycle)};
        case BVS_RELATIVE:    return BVS{.offset = (char)memory.fetch_byte_and_proceed(PC, cycle)};

        case BIT_ZERO_PAGE:   return BIT_ZeroPage{.address = memory.fetch_byte_and_proceed(PC, cycle)};
        case BIT_ABSOLUTE:    return BIT_Absolute{.address = memory.fetch_word_and_proceed(PC, cycle)};
        ...
        case CMP_IMMEDIATE:   return CMP_Immediate{.value = memory.fetch_byte_and_proceed(PC, cycle)};
        case CMP_ZERO_PAGE:   return CMP_ZeroPage{.address = memory.fetch_byte_and_proceed(PC, cycle)};
        case CMP_ZERO_PAGE_X: return CMP_ZeroPageX{.address = memory.fetch_byte_and_proceed(PC, cycle)};
        case CMP_ABSOLUTE:    return CMP_Absolute{.address = memory.fetch_word_and_proceed(PC, cycle)};
        case CMP_ABSOLUTE_X:  return CMP_AbsoluteX{.address = memory.fetch_word_and_proceed(PC, cycle)};
        case CMP_ABSOLUTE_Y:  return CMP_AbsoluteY{.address = memory.fetch_word_and_proceed(PC, cycle)};
        case CMP_INDIRECT_X:  return CMP_IndirectX{.address = memory.fetch_byte_and_proceed(PC, cycle)};
        case CMP_INDIRECT_Y:  return CMP_IndirectY{.address = memory.fetch_byte_and_proceed(PC, cycle)};

        case CPX_IMMEDIATE:   return CPX_Immediate{.value = memory.fetch_byte_and_proceed(PC, cycle)};
        case CPX_ZERO_PAGE:   return CPX_ZeroPage{.address = memory.fetch_byte_and_proceed(PC, cycle)};
        case CPX_ABSOLUTE:    return CPX_Absolute{.address = memory.fetch_word_and_proceed(PC, cycle)};

        case CPY_IMMEDIATE:   return CPY_Immediate{.value = memory.fetch_byte_and_proceed(PC, cycle)};
        case CPY_ZERO_PAGE:   return CPY_ZeroPage{.address = memory.fetch_byte_and_proceed(PC, cycle)};
        case CPY_ABSOLUTE:    return CPY_Absolute{.address = memory.fetch_word_and_proceed(PC, cycle)};

        case DEC_ZERO_PAGE:   return DEC_ZeroPage{.address = memory.fetch_byte_and_proceed(PC, cycle)};
        case DEC_ZERO_PAGE_X: return DEC_ZeroPageX{.address = memory.fetch_byte_and_proceed(PC, cycle)};
        case DEC_ABSOLUTE:    return DEC_Absolute{.address = memory.fetch_word_and_proceed(PC, cycle)};
        case DEC_ABSOLUTE_X:  return DEC_AbsoluteX{.address = memory.fetch_word_and_proceed(PC, cycle)};

        case DEX_IMPLICIT:    return DEX{};
        case DEY_IMPLICIT:    return DEY{};

        case EOR_IMMEDIATE:   return EOR_Immediate{.value = memory.fetch_byte_and_proceed(PC, cycle)};
        case EOR_ZERO_PAGE:   return EOR_ZeroPage{.address = memory.fetch_byte_and_proceed(PC, cycle)};
        case EOR_ZERO_PAGE_X: return EOR_ZeroPageX{.address = memory.fetch_byte_and_proceed(PC, cycle)};
        case EOR_ABSOLUTE:    return EOR_Absolute{.address = memory.fetch_word_and_proceed(PC, cycle)};
        case EOR_ABSOLUTE_X:  return EOR_AbsoluteX{.address = memory.fetch_word_and_proceed(PC, cycle)};
        case EOR_ABSOLUTE_Y:  return EOR_AbsoluteY{.address = memory.fetch_word_and_proceed(PC, cycle)};
        case EOR_INDIRECT_X:  return EOR_IndirectX{.address = memory.fetch_byte_and_proceed(PC, cycle)};
        case EOR_INDIRECT_Y:  return EOR_IndirectY{.address = memory.fetch_byte_and_proceed(PC, cycle)};

        case INC_ZERO_PAGE:   return INC_ZeroPage{.address = memory.fetch_byte_and_proceed(PC, cycle)};
        case INC_ZERO_PAGE_X: return INC_ZeroPageX{.address = memory.fetch_byte_and_proceed(PC, cycle)};
        case INC_ABSOLUTE:    return INC_Absolute{.address = memory.fetch_word_and_proceed(PC, cycle)};
        case INC_ABSOLUTE_X:  return INC_AbsoluteX{.address = memory.fetch_word_and_proceed(PC, cycle)};

        case INX_IMPLICIT:    return INX{};
        case INY_IMPLICIT:    return INY{};

        case JMP_ABSOLUTE:    return JMP_Absolute{.address = memory.fetch_word_and_proceed(PC, cycle)};
        case JMP_INDIRECT:    return JMP_Indirect{.address = memory.fetch_word_and_proceed(PC, cycle)};

        case JSR_ABSOLUTE:    return JSR{.address = memory.fetch_word_and_proceed(PC, cycle)};

        case LDA_IMMEDIATE:   return LDA_Immediate{.value = memory.fetch_byte_and_proceed(PC, cycle)};
        case LDA_ZERO_PAGE:   return LDA_ZeroPage{.address = memory.fetch_byte_and_proceed(PC, cycle)};
        case LDA_ZERO_PAGE_X: return LDA_ZeroPageX{.address = memory.fetch_byte_and_proceed(PC, cycle)};
        case LDA_ABSOLUTE:    return LDA_Absolute{.address = memory.fetch_word_and_proceed(PC, cycle)};
        case LDA_ABSOLUTE_X:  return LDA_AbsoluteX{.address = memory.fetch_word_and_proceed(PC, cycle)};
        case LDA_ABSOLUTE_Y:  return LDA_AbsoluteY{.address = memory.fetch_word_and_proceed(PC, cycle)};
        case LDA_INDIRECT_X:  return LDA_IndirectX{.address = memory.fetch_byte_and_proceed(PC, cycle)};
        case LDA_INDIRECT_Y:  return LDA_IndirectY{.address = memory.fetch_byte_and_proceed(PC, cycle)};

        case LDX_IMMEDIATE:   return LDX_Immediate{.value = memory.fetch_byte_and_proceed(PC, cycle)};
        case LDX_ZERO_PAGE:   return LDX_ZeroPage{.address = memory.fetch_byte_and_proceed(PC, cycle)};
        case LDX_ZERO_PAGE_Y: return LDX_ZeroPageY{.address = memory.fetch_byte_and_proceed(PC, cycle)};
        case LDX_ABSOLUTE:    return LDX_Absolute{.address = memory.fetch_word_and_proceed(PC, cycle)};
        case LDX_ABSOLUTE_Y:  return LDX_AbsoluteY{.address = memory.fetch_word_and_proceed(PC, cycle)};

        case LDY_IMMEDIATE:   return LDY_Immediate{.value = memory.fetch_byte_and_proceed(PC, cycle)};
        case LDY_ZERO_PAGE:   return LDY_ZeroPage{.address = memory.fetch_byte_and_proceed(PC, cycle)};
        case LDY_ZERO_PAGE_X: return LDY_ZeroPageX{.address = memory.fetch_byte_and_proceed(PC, cycle)};
        case LDY_ABSOLUTE:    return LDY_Absolute{.address = memory.fetch_word_and_proceed(PC, cycle)};
        case LDY_ABSOLUTE_X:  return LDY_AbsoluteX{.address = memory.fetch_word_and_proceed(PC, cycle)};

        case LSR_ACCUMULATOR: return LSR_Accumulator{};
        case LSR_ZERO_PAGE:   return LSR_ZeroPage{.address = memory.fetch_byte_and_proceed(PC, cycle)};
        case LSR_ZERO_PAGE_X: return LSR_ZeroPageX{.address = memory.fetch_byte_and_proceed(PC, cycle)};
        case LSR_ABSOLUTE:    return LSR_Absolute{.address = memory.fetch_word_and_proceed(PC, cycle)};
        case LSR_ABSOLUTE_X:  return LSR_AbsoluteX{.address = memory.fetch_word_and_proceed(PC, cycle)};

        case NOP_IMPLICIT:    return NOP{};

        case ORA_IMMEDIATE:   return ORA_Immediate{.value = memory.fetch_byte_and_proceed(PC, cycle)};
        case ORA_ZERO_PAGE:   return ORA_ZeroPage{.address = memory.fetch_byte_and_proceed(PC, cycle)};
        case ORA_ZERO_PAGE_X: return ORA_ZeroPageX{.address = memory.fetch_byte_and_proceed(PC, cycle)};
        case ORA_ABSOLUTE:    return ORA_Absolute{.address = memory.fetch_word_and_proceed(PC, cycle)};
        case ORA_ABSOLUTE_X:  return ORA_AbsoluteX{.address = memory.fetch_word_and_proceed(PC, cycle)};
        case ORA_ABSOLUTE_Y:  return ORA_AbsoluteY{.address = memory.fetch_word_and_proceed(PC, cycle)};
        case ORA_INDIRECT_X:  return ORA_IndirectX{.address = memory.fetch_byte_and_proceed(PC, cycle)};
        case ORA_INDIRECT_Y:  return ORA_IndirectY{.address = memory.fetch_byte_and_proceed(PC, cycle)};
        ...
        case ROL_ZERO_PAGE:   return ROL_ZeroPage{.address = memory.fetch_byte_and_proceed(PC, cycle)};
        case ROL_ZERO_PAGE_X: return ROL_ZeroPageX{.address = memory.fetch_byte_and_proceed(PC, cycle)};
        case ROL_ABSOLUTE:    return ROL_Absolute{.address = memory.fetch_word_and_proceed(PC, cycle)};
        case ROL_ABSOLUTE_X:  return ROL_AbsoluteX{.address = memory.fetch_word_and_proceed(PC, cycle)};

        case ROR_ACCUMULATOR: return ROR_Accumulator{};
        case ROR_ZERO_PAGE:   return ROR_ZeroPage{.address = memory.fetch_byte_and_proceed(PC, cycle)};
        case ROR_ZERO_PAGE_X: return ROR_ZeroPageX{.address = memory.fetch_byte_and_proceed(PC, cycle)};
        case ROR_ABSOLUTE:    return ROR_Absolute{.address = memory.fetch_word_and_proceed(PC, cycle)};
        case ROR_ABSOLUTE_X:  return ROR_AbsoluteX{.address = memory.fetch_word_and_proceed(PC, cycle)};

        case RTI_IMPLICIT:    return RTI{};
        case RTS_IMPLICIT:    return RTS{};

        case SBC_IMMEDIATE:   return SBC_Immediate{.value = memory.fetch_byte_and_proceed(PC, cycle)};
        case SBC_ZERO_PAGE:   return SBC_ZeroPage{.address = memory.fetch_byte_and_proceed(PC, cycle)};
        case SBC_ZERO_PAGE_X: return SBC_ZeroPageX{.address = memory.fetch_byte_and_proceed(PC, cycle)};
        case SBC_ABSOLUTE:    return SBC_Absolute{.address = memory.fetch_word_and_proceed(PC, cycle)};
        case SBC_ABSOLUTE_X:  return SBC_AbsoluteX{.address = memory.fetch_word_and_proceed(PC, cycle)};
        case SBC_ABSOLUTE_Y:  return SBC_AbsoluteY{.address = memory.fetch_word_and_proceed(PC, cycle)};
        case SBC_INDIRECT_X:  return SBC_IndirectX{.address = memory.fetch_byte_and_proceed(PC, cycle)};
        case SBC_INDIRECT_Y:  return SBC_IndirectY{.address = memory.fetch_byte_and_proceed(PC, cycle)};

        case SEC_IMPLICIT:    return SEC{};
        case SED_IMPLICIT:    return SED{};
        case SEI_IMPLICIT:    return SEI{};

        case STA_ZERO_PAGE:   return STA_ZeroPage{.address = memory.fetch_byte_and_proceed(PC, cycle)};
        case STA_ZERO_PAGE_X: return STA_ZeroPageX{.address = memory.fetch_byte_and_proceed(PC, cycle)};
        case STA_ABSOLUTE:    return STA_Absolute{.address = memory.fetch_word_and_proceed(PC, cycle)};
        case STA_ABSOLUTE_X:  return STA_AbsoluteX{.address = memory.fetch_word_and_proceed(PC, cycle)};
        case STA_ABSOLUTE_Y:  return STA_AbsoluteY{.address = memory.fetch_word_and_proceed(PC, cycle)};
        case STA_INDIRECT_X:  return STA_IndirectX{.address = memory.fetch_byte_and_proceed(PC, cycle)};
        case STA_INDIRECT_Y:  return STA_IndirectY{.address = memory.fetch_byte_and_proceed(PC, cycle)};

        case STX_ZERO_PAGE:   return STX_ZeroPage{.address = memory.fetch_byte_and_proceed(PC, cycle)};
        case STX_ZERO_PAGE_Y: return STX_ZeroPageY{.address = memory.fetch_byte_and_proceed(PC, cycle)};
        case STX_ABSOLUTE:    return STX_Absolute{.address = memory.fetch_word_and_proceed(PC, cycle)};

        case STY_ZERO_PAGE:   return STY_ZeroPage{.address = memory.fetch_byte_and_proceed(PC, cycle)};
        case STY_ZERO_PAGE_X: return STY_ZeroPageX{.address = memory.fetch_byte_and_proceed(PC, cycle)};
        case STY_ABSOLUTE:    return STY_Absolute{.address = memory.fetch_word_and_proceed(PC, cycle)};
