...
static const std::array<Addressing, 6> testedAddressings {
        // no page crossing
...
TEST_F(MOS6502_TestFixture, Test_INC) {
    for (auto value: testedInputs)
        for (const auto& addressing: testedAddressings)
            test_deincrement_memory(MOS6502_TestFixture::ChangeByOne::INCREMENT, value, addressing);
}

TEST_F(MOS6502_TestFixture, Test_DEC) {
    for (auto value: testedInputs)
        for (const auto& addressing: testedAddressings)
            test_deincrement_memory(MOS6502_TestFixture::ChangeByOne::DECREMENT, value, addressing);
}
